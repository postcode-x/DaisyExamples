...
{
  // Load the raw audio data from memory
...
double* dsp::ImpulseResponse::Process(double* inputs, const size_t numFrames)
{
  this->_PrepareBuffers(numFrames);
  this->_UpdateHistory(inputs, numFrames);

  for (size_t i = 0, j = this->mHistoryIndex - this->mHistoryRequired; i < numFrames; i++, j++)
  {
    auto input = Eigen::Map<const Eigen::VectorXf>(&this->mHistory[j], this->mHistoryRequired + 1);
    this->mOutputs[i] = (double)this->mWeight.dot(input);
  }
  // Copy out for more-than-mono.
  //for (size_t c = 1; c < numChannels; c++)
    for (size_t i = 0; i < numFrames; i++)
      this->mOutputs[i] = this->mOutputs[i];
