...
DSP_SAMPLE* dsp::DSP::_GetPointers()
{
    //for(size_t c = 0; c < this->_GetNumChannels(); c++)
        this->mOutputPointers = this->mOutputs.data();
...
void dsp::History::_EnsureHistorySize(const size_t bufferSize)
{
  ...
  const size_t requiredHistoryArraySize = 3 * repeatSize; //8614; // Just so we don't spend too much time copying back.
  if (this->mHistory.size() < requiredHistoryArraySize)
  {
    this->mHistory.resize(requiredHistoryArraySize);
    std::fill(this->mHistory.begin(), this->mHistory.end(), 0.0f); // TODO: Crashing here
...
void dsp::History::_RewindHistory()
{
    ...
    for(size_t i = 0, j = this->mHistoryIndex - this->mHistoryRequired;
        i < this->mHistoryRequired;
        i++, j++)
        this->mHistory[i] = this->mHistory[j];
    this->mHistoryIndex = this->mHistoryRequired;
}

/*void dsp::History::_UpdateHistory(DSP_SAMPLE** inputs,
...
void dsp::History::_UpdateHistory(DSP_SAMPLE* inputs, const size_t numFrames)
{
    this->_EnsureHistorySize(numFrames);
    //if(numChannels < 1)
      //  throw std::runtime_error("Zero channels?");
    ...
    for(size_t i = 0, j = this->mHistoryIndex; i < numFrames; i++, j++)
        // Convert down to float here.
        this->mHistory[j] = (float)inputs[i];
}
